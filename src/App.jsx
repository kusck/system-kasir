...
import { Package, ShoppingCart, Wallet, BarChart3, Printer, Trash2, LogOut, Lock, UtensilsCrossed } from 'lucide-react';
import './style.css';

const API = import.meta.env.VITE_API_BASE_URL || '/api';
const money = (n) => new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', maximumFractionDigits: 0 }).format(Number(n || 0));

function getSession() { try { return JSON.parse(localStorage.getItem('sk_session') || 'null'); } catch { return null; } }
function saveSession(s) { localStorage.setItem('sk_session', JSON.stringify(s)); }
...
// ── Root: hanya urus login/logout, tidak re-mount MainApp ──
function Root() {
  const [session, setSession] = useState(getSession);
  const [showLogin, setShowLogin] = useState(!getSession());
  ...
  return <MainApp key={session.id} session={session} onLogout={handleLogout} />;
}
...
// ── MainApp: semua state data ada di sini ──────────
function MainApp({ session, onLogout }) {
  const [page, setPage] = useState(session.branchId ? 'pos' : 'cash');
  ...
  const loadAll = useCallback(async () => {
    try {
      ...
      const q = [bq, rq].filter(Boolean).join('&');
      const [p, c, s, l] = await Promise.all([
        ...
        fetch(`${API}/cash/summary?${q}`).then(r => r.json()),
        fetch(`${API}/cash/logs?${q}`).then(r => r.json()),
      ]);
  ...
  async function resetCashLogs() {
    if (!confirm('Yakin reset semua catatan buku kas?')) return;
    const res = await fetch(`${API}/cash/reset`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ branchId, role: session.role }) });
    if (!res.ok) return alert('Gagal reset');
    await loadAll(); showToast('Catatan buku kas berhasil di-reset.');
  }
  ...
  function updateQty(id, qty) { setCart(prev => prev.map(i => i.id === id ? { ...i, qty: Math.max(1, Math.min(Number(qty), i.stock)) } : i)); }
  ...
  async function checkout() {
    const payload = { items: cart.map(i => ({ id: i.id, qty: i.qty })), paymentMethod, paidAmount: Number(paidAmount || 0), referenceNo, branchId };
    ...
    setReceipt(data.transaction); setCart([]); setPaidAmount(''); setReferenceNo('');
    setPrintReady(true); await loadAll(); showToast(`Transaksi ${data.transaction.invoiceNo} berhasil.`);
  }

  function printReceipt() { window.print(); setPrintReady(false); setReceipt(null); }
  ...
  return (
    <div className="app">
      ...
      <aside className="sidebar no-print">
        ...
        <button onClick={() => setPage('menu')} className={page === 'menu' ? 'active' : ''}><UtensilsCrossed size={20}/> Menu & Harga</button>
        {(isOwner || branchId) && <button onClick={() => setPage('inventory')} className={page === 'inventory' ? 'active' : ''}><Package size={20}/> Inventory</button>}
        ...
        <button onClick={onLogout} className="logout-btn"><LogOut size={20}/> Keluar</button>
      </aside>
      <nav className="mobile-nav no-print">
        {branchId && <button onClick={() => setPage('pos')} className={page === 'pos' ? 'active' : ''}><ShoppingCart size={18}/></button>}
        <button onClick={() => setPage('menu')} className={page === 'menu' ? 'active' : ''}><UtensilsCrossed size={18}/></button>
        {(isOwner || branchId) && <button onClick={() => setPage('inventory')} className={page === 'inventory' ? 'active' : ''}><Package size={18}/></button>}
        <button onClick={() => setPage('cash')} className={page === 'cash' ? 'active' : ''}><Wallet size={18}/></button>
        <button onClick={() => setPage('dashboard')} className={page === 'dashboard' ? 'active' : ''}><BarChart3 size={18}/></button>
      ...
      <main className="main no-print">
        ...
        {page === 'pos' && !branchId && <div className="panel" style={{padding:32}}><p>Owner tidak memiliki cabang kasir. Gunakan menu lain.</p></div>}
        {page === 'menu' && <MenuPage products={products} categories={categories} loadAll={loadAll} branchId={branchId} isOwner={isOwner} />}
        {page === 'inventory' && <Inventory products={products} categories={categories} loadAll={loadAll} branchId={branchId} isOwner={isOwner} />}
...
// ── POS Page ─────────────────────────────────────────
function POSPage({ products, categories, query, setQuery, cart, addToCart, updateQty, setCart, barcodeRef, handleBarcode, total, paymentMethod, setPaymentMethod, paidAmount, setPaidAmount, referenceNo, setReferenceNo, change, checkout, printReady, printReceipt }) {
  return (
    <div>
      ...
      <div className="pos-grid">
        ...
        <section className="panel cart-panel">
          ...
          <select value={paymentMethod} onChange={e => setPaymentMethod(e.target.value)}>
            ...
            <option value="TRANSFER">Transfer Bank</option><option value="CARD">Debit/Kredit</option>
          </select>
          {paymentMethod === 'CASH'
...
function MenuPage({ products, categories, loadAll, branchId, isOwner }) {
  ...
  function handleName(val) {
    setForm(prev => ({ ...prev, name: val, sku: prev.sku || generateSKU(val), barcode: prev.barcode || generateBarcode() }));
  }

  async function save(e) {
    e.preventDefault(); setSaving(true);
    const url = `${API}/products${editingId ? '/' + editingId : ''}?branchId=${branchId}`;
    const res = await fetch(url, { method: editingId ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...form, branchId }) });
    const data = await res.json();
    setSaving(false);
  ...
  async function del(id) {
    ...
    await fetch(`${API}/products/${id}?branchId=${branchId}`, { method: 'DELETE' });
    loadAll();
  }
  ...
  async function saveCat(e) {
    ...
    await fetch(`${API}/categories?branchId=${branchId}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: newCat.trim(), branchId }) });
    setNewCat(''); setShowCatForm(false); loadAll();
  }

  const filtered = filterCat ? products.filter(p => p.categoryId === Number(filterCat)) : products;
  const grouped = categories.map(cat => ({ ...cat, items: filtered.filter(p => p.categoryId === cat.id) })).filter(g => g.items.length > 0);
  const uncategorized = filtered.filter(p => !p.categoryId);

  return (
    <div>
      <div className="topbar"><h2>🍽️ Menu & Harga</h2><span>{products.length} menu tersedia</span></div>
      ...
      {isOwner && (
        <div className="panel inv-form-panel">
          <div className="inv-form-header">
            ...
            <button type="button" className="btn-secondary" onClick={() => setShowCatForm(v => !v)}>+ Kategori</button>
          </div>
          {showCatForm && (
            <form className="cat-form" onSubmit={saveCat}>
              <input value={newCat} onChange={e => setNewCat(e.target.value)} placeholder="Nama kategori baru" required autoFocus />
          ...
          <form onSubmit={save}>
            <div className="inv-grid">
              <div className="inv-field full">
                ...
                <input value={form.name} onChange={e => handleName(e.target.value)} placeholder="Contoh: Nasi Timbel Spesial" required />
              </div>
              ...
              <div className="inv-field">
                ...
                <div className="input-with-btn">
                  <input value={form.sku} onChange={e => setForm({...form, sku: e.target.value})} placeholder="Auto dari nama" />
                  <button type="button" className="btn-regen" onClick={() => setForm(p => ({...p, sku: generateSKU(p.name)}))}>🔄</button>
                </div>
              </div>
            </div>
            <div className="inv-actions">
              <button className="checkout" type="submit" disabled={saving}>{saving ? 'Menyimpan...' : editingId ? '💾 Update Menu' : '✅ Tambah Menu'}</button>
      ...
      <div className="panel" style={{display:'flex',gap:'8px',flexWrap:'wrap',alignItems:'center',padding:'12px 16px',marginBottom:4}}>
        <span style={{fontWeight:600,fontSize:13,color:'#374151'}}>Filter:</span>
        <button className={`cat-chip ${!filterCat ? 'active' : ''}`} onClick={() => setFilterCat('')}>Semua ({products.length})</button>
        {categories.map(c => <button key={c.id} className={`cat-chip ${filterCat === String(c.id) ? 'active' : ''}`} onClick={() => setFilterCat(String(c.id))}>{c.name}</button>)}
      </div>

      {grouped.map(group => (
        <div key={group.id} className="panel" style={{marginBottom:12}}>
          <h3 style={{margin:'0 0 12px',color:'var(--primary)',borderBottom:'2px solid #e2e8f0',paddingBottom:'8px'}}>{group.name} <span style={{color:'#9ca3af',fontWeight:400,fontSize:13}}>({group.items.length})</span></h3>
          <div className="menu-grid">
            {group.items.map(p => (
              <div key={p.id} className="menu-card">
                ...
                {isOwner && <div className="menu-card-actions"><button className="btn-edit-sm" onClick={() => { setEditingId(p.id); setForm({...p, categoryId: p.categoryId || ''}); window.scrollTo(0,0); }}>Edit</button><button className="btn-del-sm" onClick={() => del(p.id)}>Hapus</button></div>}
              </div>
            ))}
          </div>
        </div>
      ...
      {uncategorized.length > 0 && (
        <div className="panel" style={{marginBottom:12}}>
          <h3 style={{margin:'0 0 12px',color:'#6b7280'}}>Tanpa Kategori ({uncategorized.length})</h3>
          <div className="menu-grid">
            {uncategorized.map(p => (
              <div key={p.id} className="menu-card">
                <div className="menu-card-name">{p.name}</div>
                <div className="menu-card-price">{money(p.sellingPrice)}</div>
                <div className="menu-card-meta">Modal: {money(p.costPrice)} • Stok: {p.stock}</div>
                {isOwner && <div className="menu-card-actions"><button className="btn-edit-sm" onClick={() => { setEditingId(p.id); setForm({...p, categoryId: ''}); window.scrollTo(0,0); }}>Edit</button><button className="btn-del-sm" onClick={() => del(p.id)}>Hapus</button></div>}
              </div>
            ))}
          </div>
        </div>
      )}
      {filtered.length === 0 && <div className="panel" style={{textAlign:'center',color:'#9ca3af',padding:'40px'}}>{isOwner ? 'Belum ada menu. Tambahkan di atas.' : 'Belum ada menu.'}</div>}
    </div>
  );
}
...
function Inventory({ products, categories, loadAll, branchId, isOwner }) {
  ...
  async function del(id) {
    ...
    if (confirm('Hapus barang?')) { await fetch(`${API}/products/${id}?branchId=${branchId}`, { method: 'DELETE' }); loadAll(); }
  }

  return (
    <div>
      <div className="topbar"><h2>Inventory</h2><span>Kelola stok produk</span></div>
      {isOwner && (
        <div className="panel inv-form-panel">
          <h3 style={{margin:'0 0 16px'}}>{editingId ? '✏️ Edit Produk' : '➕ Tambah Produk'}</h3>
          <form onSubmit={save}>
            <div className="inv-grid">
              <div className="inv-field full"><label>Nama Produk <span className="req">*</span></label><input value={form.name} onChange={e => handleName(e.target.value)} placeholder="Nama produk" required /></div>
              <div className="inv-field"><label>SKU <span className="auto-tag">otomatis</span></label><div className="input-with-btn"><input value={form.sku} onChange={e => setForm({...form, sku: e.target.value})} placeholder="SKU" /><button type="button" className="btn-regen" onClick={() => setForm(p => ({...p, sku: generateSKU(p.name)}))}>🔄</button></div></div>
              <div className="inv-field"><label>Barcode <span className="auto-tag">otomatis</span></label><div className="input-with-btn"><input value={form.barcode} onChange={e => setForm({...form, barcode: e.target.value})} placeholder="Barcode" /><button type="button" className="btn-regen" onClick={() => setForm(p => ({...p, barcode: generateBarcode()}))}>🔄</button></div></div>
              <div className="inv-field"><label>Kategori</label><select value={form.categoryId} onChange={e => setForm({...form, categoryId: e.target.value})}><option value="">Pilih</option>{categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
              <div className="inv-field"><label>Harga Modal <span className="req">*</span></label><input type="number" value={form.costPrice} onChange={e => setForm({...form, costPrice: e.target.value})} placeholder="0" required min="0" /></div>
              <div className="inv-field"><label>Harga Jual <span className="req">*</span></label><input type="number" value={form.sellingPrice} onChange={e => setForm({...form, sellingPrice: e.target.value})} placeholder="0" required min="0" /></div>
              <div className="inv-field"><label>Stok <span className="req">*</span></label><input type="number" value={form.stock} onChange={e => setForm({...form, stock: e.target.value})} placeholder="0" required min="0" /></div>
              <div className="inv-field"><label>Stok Minimum</label><input type="number" value={form.minStock} onChange={e => setForm({...form, minStock: e.target.value})} placeholder="5" min="0" /></div>
            </div>
            <div className="inv-actions">
              <button className="checkout" type="submit">{editingId ? '💾 Update' : '✅ Simpan'}</button>
              {editingId && <button type="button" className="btn-secondary" onClick={() => { setForm(blank); setEditingId(null); }}>Batal</button>}
            </div>
          </form>
        </div>
      ...
      <div className="panel table-wrap">
        <div className="table-toolbar"><h3 style={{margin:0}}>Daftar Produk ({products.length})</h3></div>
        <table><thead><tr><th>Nama</th><th>Kategori</th><th>SKU</th><th>Barcode</th><th>Harga Jual</th><th>Stok</th>{isOwner && <th>Aksi</th>}</tr></thead>
          <tbody>{products.map(p => (
            <tr key={p.id}>
              ...
              {isOwner && <td><button onClick={() => { setEditingId(p.id); setForm({...p, categoryId: p.categoryId || ''}); }}>Edit</button><button onClick={() => del(p.id)}>Hapus</button></td>}
            </tr>
          ))}</tbody>
        </table>
      </div>
...
function CashPage({ cashLogs, loadAll, resetCashLogs, branchId, isOwner, session }) {
  ...
  const filteredLogs = useMemo(() => filter === 'ALL' ? cashLogs : cashLogs.filter(l => l.type === filter), [cashLogs, filter]);
  ...
  async function save(e) {
    ...
    const res = await fetch(endpoint, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ amount: finalAmount, description, source: source || (mode === 'IN' ? 'PENJUALAN' : 'BIAYA'), branchId, role: session.role }) });
    const data = await res.json();
    if (!res.ok) return alert(data?.message || 'Gagal menyimpan');
    setAmount(''); setSource(''); setDescription(''); await loadAll();
  }
  ...
  return (
    <div>
      <div className="topbar">
        ...
        <button className="checkout" style={{background:'#dc2626',borderRadius:'12px'}} onClick={resetCashLogs}>Reset Kas</button>
      </div>
      ...
      <div className="panel cash-log-header table-toolbar">
        ...
        <select className="cash-filter" value={filter} onChange={e => setFilter(e.target.value)}><option value="ALL">Semua</option><option value="IN">Masuk</option><option value="OUT">Keluar</option></select>
      </div>
      <div className="panel table-wrap">
        <table><thead><tr><th>Tanggal</th>{isOwner && <th>Cabang</th>}<th>Tipe</th><th>Sumber</th><th>Jumlah</th><th>Keterangan</th></tr></thead>
          <tbody>{filteredLogs.map(l => (
            <tr key={l.id}>
              ...
              <td>{l.source}</td><td>{money(l.amount)}</td><td>{l.description}</td>
            </tr>
          ))}</tbody>
        </table>
      </div>
...
function Dashboard({ summary, products, cashLogs, resetCashLogs, isOwner }) {
  ...
  return (
    <div>
      ...
      {products.length > 0 && (
        <div className="panel"><h3 style={{marginTop:0}}>⚠️ Low Stock Alert</h3>
